...
use crate::generated::errors::DocumentWalletError;
use crate::generated::instructions::DocumentWalletInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = DocumentWalletInstruction::unpack(data)?;

        match instruction {
			DocumentWalletInstruction::Mint(args) => {
			...
			DocumentWalletInstruction::Transfer => {
				msg!("Instruction: Transfer");
				Self::process_transfer(program_id, accounts)
			}
			DocumentWalletInstruction::Burn => {
...
/// - additional_info: [String] 
	pub fn process_mint(
		...
		let document_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (document_pubkey, document_bump) = Pubkey::find_program_address(
			&[b"document_wallet", mint_info.key.as_ref()],
			program_id,
		);

		// Security Checks
		if fee_payer_info.is_signer != true {
			return Err(DocumentWalletError::InvalidSignerPermission.into());
		}
		...
		if *document_info.key != document_pubkey {
			return Err(DocumentWalletError::NotExpectedAddress.into());
		}
		...
		let space = DocumentMetadata::LEN;
		let rent = Rent::get()?;
		let rent_minimum_balance = rent.minimum_balance(space);
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), document_info.clone()],
			&[&[b"document_wallet", mint_info.key.as_ref(), &[document_bump]]],
		)?;


		// Security Checks
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(DocumentWalletError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(DocumentWalletError::InvalidAccountLen.into());
		}

		if document_info.data_len() != DocumentMetadata::LEN {
			return Err(DocumentWalletError::InvalidAccountLen.into());
		}
		...
		let document = &mut AccountPDA::new(
			&document_info,
			try_from_slice_unchecked::<DocumentMetadata>(&document_info.data.borrow()).unwrap(),
			document_bump,
		);
		...
		document.data.serialize(&mut &mut document_info.data.borrow_mut()[..])?;
