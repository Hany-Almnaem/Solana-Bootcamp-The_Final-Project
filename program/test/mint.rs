...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("document_wallet", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let document_type: String = Default::default();
		let issuer: String = Default::default();
		let issuance_date: String = Default::default();
		let expiration_date: String = Default::default();
		let additional_info: String = Default::default();
		...
		let (document_pubkey, _) = Pubkey::find_program_address(
			&[b"document_wallet", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(document_pubkey, false),
		...
		let data = DocumentWalletInstruction::Mint(crate::generated::instructions::MintArgs{
			document_type,
			issuer,
			issuance_date,
			expiration_date,
