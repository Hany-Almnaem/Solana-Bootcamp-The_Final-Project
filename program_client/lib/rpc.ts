 ...
 *
 * Data:
 * - document_type: {@link string} 
 * - issuer: {@link string} 
 * - issuance_date: {@link string} 
 * - expiration_date: {@link string} 
 * - additional_info: {@link string} 
 */
    Mint = 0,
...
export const mint = (args: MintArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: DocumentWalletInstruction.Mint,
            document_type: args.documentType,
            issuer: args.issuer,
            issuance_date: args.issuanceDate,
            expiration_date: args.expirationDate,
            additional_info: args.additionalInfo,
        }
    );

    const [documentPubkey] = pda.deriveDocumentMetadataPDA({
        mint: args.mint,
    }, _programId);
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            {pubkey: documentPubkey, isSigner: false, isWritable: true},
...
export const transfer = (args: TransferArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: DocumentWalletInstruction.Transfer,
        }
    );
...
export const burn = (args: BurnArgs): TransactionInstruction => {
    const data = serialize(
        ...
        {
            id: DocumentWalletInstruction.Burn,
        }
    );
...
export const getDocumentMetadata = async (
...
): Promise<T.DocumentMetadata | undefined> => {
    ...
    return T.decodeDocumentMetadata(deserialize(T.DocumentMetadataSchema, buffer.data) as Record<string, unknown>);
}
